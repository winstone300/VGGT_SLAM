...
@torch.inference_mode
def vggt_asymmetric_inference(model,images):
    dtype = torch.float16
    ...
    X = predictions["world_points"].squeeze(0)
    C = predictions["world_points_conf"].squeeze(0)
    D = predictions["depth"].squeeze(0)
    Q = predictions["depth_conf"].squeeze(0) 
    return X, C, D, Q

def vggt_match_asymmetric(model, images, idx_i2j_init=None):
    X, C, D, Q = vggt_asymmetric_inference(model,images)
...
def ImgNorm(img_tensor):
    # Normalize image tensor from [0, 1] to [-1, 1]
    return img_tensor * 2 - 1


def resize_img_Vggt(img,target_size,mode='crop'):
    img = Image.open(img)
