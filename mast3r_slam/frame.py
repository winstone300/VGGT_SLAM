...
import lietorch
import torch
from mast3r_slam.mast3r_utils import resize_img,resize_img_Vggt
...
def create_frame_Vggt(i, img, T_WC, img_size=518, device="cuda:0"):
    img = resize_img_Vggt(img, img_size)
    rgb = img["img"].to(device=device)
    img_shape = torch.tensor(img["true_shape"], device=device)
    img_true_shape = img_shape.clone()
    uimg = torch.from_numpy(img["unnormalized_img"]) / 255.0
